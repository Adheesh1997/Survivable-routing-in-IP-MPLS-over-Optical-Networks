...
protected:
	...
	vector<int> forNoLSPaverage;
	vector<int> forLSPbandAverage;
	int backUpLPavaialablecount;
	int rejectedAvailbackupLPcount;

	vector<float> forAvrgHLLPprobability;
...
public:
	...
	void setANewLighpath(vector<int> shortestPath, int wavelengthSt, string type, int LPidentifier, int bandwidthCap); //Establish a new lighpath
	...
	vector<float> averageCalculation();
	void heavilyLPprotectionCount(thresholds thresholdVals);
	vector<float> heavilyLPAndOtherAvgCalc();
