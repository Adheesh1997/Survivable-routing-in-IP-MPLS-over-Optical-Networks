...
void waveLengthNetworks::printAdjacency()
{
    for (size_t i = 0; i < waveAdjacancyMatrix.size(); i++)
    {
        cout << i << "-->  ";
        for (size_t j = 0; j < waveAdjacancyMatrix[i].size(); j++)
            cout << waveAdjacancyMatrix[i][j] << "  ";
        cout << endl;
    }
}
